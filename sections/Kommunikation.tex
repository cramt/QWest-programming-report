...
I QWest.DataAccess.ConnectionWrapper har vi hjælpemetoden \texttt{Use} \ref{lst:connectionWrapperUse}, denne metode åbner en forbindelse til databasen, og starter en \texttt{SQLCommand} baseret på det første argument i metoden, SQL query'en. en \texttt{SQLCommand} er automatisk i en transaction lige meget hvor mange query'er der er i query strengen. Det andet argument er en funktionen givet af kalderen, denne bliver eksekveret med \texttt{SQLComamnd}'en og så snart den er færdig lukkes forbindelsen. Et eksempel på brugen af Use metode kan findes i Update DAO metoden for Posts \ref{lst:SQLinjection}. Inde i scopet af funktionen givet til Use metoden er forbindelsen åben, uden for er den ikke.
