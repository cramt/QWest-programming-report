...
Efter migrationsprocessen i QWest, indsættes også backup af geopolitisk data,hvis intet geopolitisk data kan findes i databasen. Dette skyldes tildels testing årsager, eftersom vores integration-tests kan finde på at slette alle tabeller i databasen, hvilket ville slette denne næsten statiske data.  %tildels pga. af at dette skulle uploades til databasen første gang på en eller anden måde.

\section{Services}\label{sec:servicesImp}
QWest er en applikation bestående af services, enten skrevet i node.js\cite{nodejs} eller C\#. Som nævnt i \ref{sec:servicesArc} er disse services henholdsvis \texttt{QWest.Api}, \texttt{QWest.Web}, \texttt{QWest.Email}.% og \texttt{QWest.Admin}. ikke en service, right? Den er ikke nævnt som service i Arkitektur (linje 80)
...
I dette projekt er der gjort brug af softwaretests med NUnit 3\cite{nunit}. Disse tests har to formål: at dokumentere koden og hvad den kan, samt at sikre at koden virker. Vi har primært udviklet database-integrationstests, idet vores DAO-metoder indeholder meget kompleks logik i forhold til hvilke colonner og rækker, der bliver hentet fra databasen, og hvordan resultaterne bliver omdannet til model-objekter.
